...
const remoteSshWs = new RegExp('^cursor-remote://ssh-remote+(.+?(?=/))(.+)$');
const remoteWslWs = new RegExp('^cursor-remote://wsl+(.+?(?=/))(.+)$');
const codespacesWs = new RegExp('^cursor-remote://vsonline+(.+?(?=/))(.+)$');
const devContainerWs = new RegExp(
  '^cursor-remote://dev-container+(.+?(?=/))(.+)$'
);
...
function processRow(row) {
  ...
  return entries.map((item) => {
    ...
    // {
    //   "folderUri": "cursor-remote://ssh-remote%something/root/something/something",
...
async function getProjects(storagePath) {
  hlpLog('storagePath', storagePath);
  const row = await queryProjects(storagePath);
